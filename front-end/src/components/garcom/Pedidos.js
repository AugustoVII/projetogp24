...
function Pedidos() {
  ...
  const handleConfirm = () => {
    ...
    axios.post('/marcarpedidoentregue', formData)
      .then(response => {
        ...
        if (response.status === 200) { // Verifique o status 200 para sucesso
          ...
          setMensagem('Erro ao marcar o pedido como entregue. Tente novamente.');
        }
      })
      .catch(error => {
  ...
  useEffect(() => {
    const fetchPedidos = async () => {
      try {
        const response = await axios.get('/pedidopronto');
    ...
    <div className={styles.divPrinc}>
      ...
      {showModal && (
        <Modal show={showModal} handleClose={handleClose} handleConfirm={handleConfirm}>
          <p className={styles.titulo}>Tem certeza que deseja marcar pedido como entregue?</p>
        </Modal>
      )}
    </div>
  );
